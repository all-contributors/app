...
describe("parseComment", () => {
  ...
  test("Basic intent to add - username with dash", () => {
    expect(
      parseComment(`@${testBotName} Please add @Otto-J for doc, `)
    ).toEqual({
      action: "add",
      contributors: {
        "Otto-J": ["doc"],
  ...
  test("Basic intent to add - username with dash", () => {
    expect(
      parseComment(
        `@${testBotName} please add @Dev-Soumyaranjan  for infrastructure, tests and code ,`
      )
    ).toEqual({
      action: "add",
      contributors: {
        "Dev-Soumyaranjan": ["infra", "test", "code"],
      },
    });
  });

  test("Basic intent to add - with plurals", () => {
    expect(parseComment(`@${testBotName} please add dat2 for docs`)).toEqual({
      action: "add",
      contributors: {
        dat2: ["doc"],
      },
  ...
  test("Add multiple when not spaced (just split via commas)", () => {
    expect(
      parseComment(`@${testBotName} please add @stevoo24 for code,content`)
    ).toEqual({
      action: "add",
      contributors: {
  ...
  test(`Interpret users who's names are contributions`, () => {
    expect(parseComment(`@${testBotName} please add @ideas for ideas`)).toEqual(
      {
        action: "add",
        contributors: {
          ideas: ["ideas"],
  ...
  test("Support adding people with mentions", () => {
    expect(
      parseComment(`@${testBotName} please add @sinchang for infrastructure`)
    ).toEqual({
      action: "add",
      contributors: {
        sinchang: ["infra"],
  ...
  // TODO: Looks like this one is gramatically incorrect
  ...
  test("Add multiple users in 1 hit - from seperate lines with complex text at the beginning", () => {
    expect(
      parseComment(
        `
          ...
          @all-contributors please add @lksmrqrdt for bug
        `
      )
    ).toEqual({
      action: "add",
  ...
  test("Intent unknown", () => {
    expect(parseComment(`@${testBotName} please lollmate for tool`)).toEqual({
      action: false,
    });
  });

  test("Intent unknown - incomplete `add` action", () => {
    expect(parseComment(`@${testBotName} add`)).toEqual({
      action: false,
    });
  });
});
