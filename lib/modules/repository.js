...
const convertMessage = require("./convert-message");
...
class Repository {
  ...
  async updateFile({ filePath, content, branchName, originalSha }) {
    ...
    await this.octokit.repos.createOrUpdateFileContents({
      ...
      repo: this.repo,
      path: filePath,
      message: convertMessage({ tag: 'docs', msg: `update ${filePath} ${this.skipCiString}` }).trim(),
  ...
  async createFile({ filePath, content, branchName }) {
    ...
    await this.octokit.repos.createOrUpdateFileContents({
      ...
      repo: this.repo,
      path: filePath,
      message: convertMessage({ tag: 'docs', msg: `create ${filePath} ${this.skipCiString}` }).trim(),
      content: contentBinary,
      branch: branchName,
    });
  }
