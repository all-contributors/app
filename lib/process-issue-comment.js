...
async function processIssueComment({ context, commentReply }) {
  ...
  if (action === "add") {
    ...
    const branchName = `all-contributors/refresh-contributors-table`;

    await triggerActionRefresh(context, commentReply, log, branchName);
  } else {
...
async function triggerActionRefresh(context, commentReply, log, branchName) {
  // set up repository instance. Uses branch if it exists, falls back to repository's default branch
  const repository = await setupRepository({ context, branchName });
  ...
  const prContentTitle = `refresh the contributors table`;
  const prContentMessage = `Refreshes the contributors table.\n\nThis was requested by ${commentReply.replyingToWho()} [in this comment](${commentReply.replyingToWhere()})`

  const { pullCreated } = await handlePullRequest({
    context,
    commentReply,
    repository,
    config,
  ...
  log.info(
    ...
    pullCreated ? `Contributors table refreshed` : `Cannot refresh the contributors table`
  );
}
