...
async function processIssueComment({ context, commentReply }) {
  ...
  for (var k in contributors) {
    who = k
    contributions = contributors[who]

    await triggerActionAdd(context, commentReply, who, contributions)
  }
}

async function triggerActionAdd(context, commentReply, who, contributions) {
  const action = "add"
